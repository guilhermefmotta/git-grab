...
pub async fn get_branches(repo_path: String) -> Result<Vec<BranchInfo>, String> {
    ...
    for branch_type in [BranchType::Local, BranchType::Remote] {
            ...
            .filter_map(|b| b.ok())
        {
            ...
            let last_commit_hash = b.get().target().map(|oid| oid.to_string());
            ...
            let (upstream, ahead, behind) = if !is_remote {
                ...
                let (a, bh) = if let (Some(_), Some(local_oid)) = (&upstream_name, b.get().target()) {
                    b.upstream()
                        .ok()
...
pub async fn checkout_branch(repo_path: String, name: String) -> Result<(), String> {
    ...
    let branch  = repo.find_branch(&name, BranchType::Local).map_err(|e| e.message().to_string())?;
    let ref_name = branch.get().name().ok_or("invalid branch ref")?.to_string();
...
#[tauri::command]
pub async fn delete_branch(repo_path: String, name: String, force: Option<bool>) -> Result<(), String> {
    let repo = Repository::open(&repo_path).map_err(|e| e.message().to_string())?;
    let mut branch = repo.find_branch(&name, BranchType::Local).map_err(|e| e.message().to_string())?;
...
#[tauri::command]
pub async fn rename_branch(repo_path: String, name: String, new_name: String) -> Result<(), String> {
    let repo = Repository::open(&repo_path).map_err(|e| e.message().to_string())?;
    let mut branch = repo.find_branch(&name, BranchType::Local).map_err(|e| e.message().to_string())?;
...
#[tauri::command]
pub async fn rebase_branch(repo_path: String, branch_name: String) -> Result<MergeOutcome, String> {
    let repo = Repository::open(&repo_path).map_err(|e| e.message().to_string())?;
    ...
    let sig    = git2::Signature::now(&uname, &email).map_err(|e| e.message().to_string())?;

    let branch   = repo.find_branch(&branch_name, BranchType::Local).map_err(|e| e.message().to_string())?;
    let upstream = repo.reference_to_annotated_commit(branch.get()).map_err(|e| e.message().to_string())?;
    let mut rebase = repo.rebase(None, Some(&upstream), None, None).map_err(|e| e.message().to_string())?;
        ...
        match rebase.next() {
            ...
            Some(Ok(_)) => {
                ...
                if index.has_conflicts() {
                    // Don't abort — leave in REBASING state so user can resolve
                    let conflicts = super::merge::conflicted_paths(&repo)?;
                    return Ok(MergeOutcome { fast_forward: false, conflicted_files: conflicts });
                }
    ...
    Ok(MergeOutcome { fast_forward: false, conflicted_files: vec![] })
}
